...
import { FileCheck, Plus, Search, Filter, Edit2, Trash2, ExternalLink, AlertTriangle } from 'lucide-react';
...
const VehicleDocuments = () => {
  ...
  const [selectedDocument, setSelectedDocument] = useState<VehicleDocument | undefined>();
  const [loading, setLoading] = useState<string | null>(null);

  // Fetch vehicle documents from Supabase
  const { data: documents = [], isLoading } = useQuery({
    queryKey: ['vehicle-documents', user?.companyId],
    queryFn: async () => {
  ...
  const filteredDocuments = documents.filter(document => {
    const matchesSearch = 
      ...
      document.vehicle?.brand.toLowerCase().includes(searchTerm.toLowerCase()) ||
      document.vehicle?.model.toLowerCase().includes(searchTerm.toLowerCase()) ||
      document.vehicle?.plate.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleDeleteDocument = async (document: VehicleDocument) => {
    ...
    try {
      ...
      if (document.file_url.includes(supabase.supabaseUrl)) {
        try {
          await deleteFile(document.file_url, 'vehicle-documents');
        } catch (fileError) {
      ...
      await queryClient.invalidateQueries({ queryKey: ['vehicle-documents'] });
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Documentos de Veículos</h1>
          <p className="mt-1 text-sm text-gray-500">
            Gerencie documentos e vencimentos dos veículos
          </p>
        </div>
                ...
                <td className="whitespace-nowrap px-6 py-4">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <div className="text-sm text-gray-500">{document.vehicle?.plate}</div>
                    </div>
                  </div>
                </td>
                <td className="whitespace-nowrap px-6 py-4">
